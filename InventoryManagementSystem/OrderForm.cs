...
namespace InventoryManagementSystem
{
    public partial class OrderForm : Form
    {
        ...
        public void LoadOrder()
        {
            ...
            int i = 0;
            dgvOrder.Rows.Clear();
            // Sada koristi VIEW umesto direktnog JOIN-a
            cm = new SqlCommand("SELECT orderid, odate, pid, pname, cid, cname, qty, price, total FROM vw_OrderDetails WHERE CONCAT(orderid, odate, pid, pname, cid, cname, qty, price) LIKE '%" + txtSearch.Text + "%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
